...
const Status = ({ statusToFind }: { statusToFind: Task["status"] }) => {
  const status = statuses.find((status) => status.value === statusToFind)
...
export function Editor({ task }: EditorProps) {
  const { register, handleSubmit } = useForm<FormData>({
    resolver: zodResolver(
      insertTaskSchema.partial().extend({
  ...
  const initializeEditor = React.useCallback(async () => {
    ...
    const body = insertTaskSchema.parse(task)
      ...
      <div className="grid w-full gap-10">
        <div className="flex w-full items-center justify-between">
          <div className="flex items-center space-x-10">
            ...
            <Status statusToFind={task.status} />
          </div>
          {/* <button
            type="button"
            onClick={() => onDelete()}
